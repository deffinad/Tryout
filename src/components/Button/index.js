...
const Button = ({
    ...
    return (
        ...
        <Fragment>
            {!children ? (
                <button onClick={onClick} type="button" className={`text-${textColor} hover:text-${textColorHover} bg-${bgColor} hover:bg-${bgColorHover} font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 mb-2`}>
                    {icon !== '' && iconPlacement === 'start' &&
                        { icon }
                    }
